module Whirpool 
	...
	digestbytes = 64
	digestbits = digestbytes * 8
	byteslength = 32
	bitslength = byteslength * 8
	blockbytes = 64
	blockbits = blockbytes * 8
	
	function LL(x::UInt64)
		return x
	end
	
	function whirpool(file::IO)
	...
	function whirpool(msg::Array{UInt8})
	   # 
		...
		while length(msg) % (512 / 8) < ((512-64) / 8)
			push!(msg, 0)
		end
		
		#add original msg size in bits as 256 bits
		byte::UInt8 = 0
		mask::UInt64 = 0xff
		for i in 8:-1:1
		...
		while length(msg) % (512 / 8) != (0)
			#push!(msg, 0)
		end
		  
		K = zeros(UInt64, 8)
		block = zeros(UInt64, 8)
		state = zeros(UInt64, 8)
		L = zeros(UInt64, 8)
		buffer = msg
		...
		for i in 1:8
			block[i] = 
			(buffer[bufferoffset + 0] << 56) $
			((buffer[bufferoffset + 1] & mask) << 48) $
			((buffer[bufferoffset + 2] & mask) << 40) $
			((buffer[bufferoffset + 3] & mask) << 32) $
			((buffer[bufferoffset + 4] & mask) << 24) $
			((buffer[bufferoffset + 5] & mask) << 16) $
			((buffer[bufferoffset + 6] & mask) << 8) $
			((buffer[bufferoffset + 7] & mask))
			#block[i] = reversebytes(block[i])
		end
		
		for j in 1:8
			tmp::UInt64 = 0
			for k in 1:8
				tmp = (tmp << 8) | msg[1 + (j - 1)*8+8-k]
			end
		...
		for i in 1:10
			for j in 1:8
				L[j] = applaykey(K, generateindexarr(j-1) )
			end
			L[1] = L[1] $ rc[i+1]
			...
			for j in 1:8
				L[j] = applaykey(state, generateindexarr(j-1)) $ K[j]
				#state[j] = L[j]
			end
	...
	function reversebytes(x::UInt64)
		return ((x & 0x00000000000000ff)  << 56) | (x >> 56) |
	...
	function applaykey( K::Array{UInt64}, indexes::Array{})
		L = C0[(K[indexes[1]] >> 56) + 1] $
		C1[ ((K[indexes[2]] >> 48 ) & 0xff ) + 1] $
		C2[ ((K[indexes[3]] >> 40 ) & 0xff ) + 1] $
		C3[ ((K[indexes[4]] >> 32 ) & 0xff ) + 1] $
		C4[ ((K[indexes[5]] >> 24 ) & 0xff ) + 1] $
		C5[ ((K[indexes[6]] >> 16 ) & 0xff ) + 1] $
		C6[ ((K[indexes[7]] >> 8 ) & 0xff) + 1] $
		C7[ ((K[indexes[8]]) & 0xff) + 1]
		
		return L
	end

	function u32tou8(x::UInt32)
		x &= 0xff
		return convert(UInt8, x)
	end

	function u64tou8(x::UInt64)
		x &= 0xff
		return convert(UInt8, x)
	end

	function generateindexarr(x)
		arr = [1, 8, 7, 6, 5, 4, 3, 2]
		for i in 1:x
			a = arr[8]
			for j in 8:-1:2
